...
colnames(raw_inflow_data)=c("Month","Year", "HP1", "HP2","HP3","HP4", "HP5","HP6","HP7", "HP8","HP9","HP10", "HP11","HP12","HP13", "HP14","HP15")
...
```{r}
ts_inflow_data <- ts(inflow_data[,2:(2+nhydro-1)],frequency=12) #note that we are only transforming columns with inflow data, not the date columns  #start=my_date[1],end=my_date[nobs],
...
for(i in 1:nhydro){
  ...
  Acf(ts_inflow_data[,i],lag.max=40,main=paste("Inflows HP",i,sep="")) # because I am not storing Acf() into any object, I don't need to specify plot=TRUE 
...
dummies <- seasonaldummy(ts_inflow_data[,iHP])  #this function only accepts ts object, no need to add one here because date object is not a column
